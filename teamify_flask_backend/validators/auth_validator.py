...
class RegisterSchema(Schema):
    ...
    role = fields.Str(
        load_default="member", 
        validate=validate.OneOf(["member", "guest", "admin"], error="Invalid role")
    )
    user_type = fields.Str(
        load_default=None, 
        validate=validate.OneOf(["freelancer", "student", "guest"], error="Invalid user_type")
    )
    ...
    def validate_password(self, value):
        if not PASSWORD_RE.match(value):
            raise ValidationError("Password must be at least 8 characters with 1 uppercase letter and 1 digit")
...
class ProfileUpdateSchema(Schema):
    ...
    user_type = fields.Str(validate=validate.OneOf(["freelancer", "student", "guest"], error="Invalid user_type"))
