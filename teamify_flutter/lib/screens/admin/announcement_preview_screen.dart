...
class AnnouncementPreviewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final item = ModalRoute.of(context)?.settings.arguments as AdminAnnouncement?;
        ...
        children: [
            ...
            child: Column(
              children: [
                ...
                _summaryRow(
                  ...
                  value: item.audience == AnnouncementAudience.allUsers
                      ? '~2,450 active users'
                      : '~450 users in segment',
                ),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: TButton(
                  label: item.status == AnnouncementStatus.scheduled
                      ? 'Confirm & Schedule'
                      : 'Confirm & Send',
                  onTap: () {
                    final targetStatus = item.scheduledAt != null
                        ? AnnouncementStatus.scheduled
                        : AnnouncementStatus.sent;
                    DemoAnnouncementStore.instance.add(
                      item.copyWith(
                        status: targetStatus,
                        sentAt: targetStatus == AnnouncementStatus.sent
                            ? DateTime.now()
                            : null,
                      ),
                    );
                    Navigator.popUntil(
                      context,
                      ModalRoute.withName(R.adminAnnouncements),
                    );
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(targetStatus == AnnouncementStatus.scheduled
