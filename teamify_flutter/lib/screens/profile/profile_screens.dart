...
import '../../data/demo/demo_universities_data.dart';
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void _initUniversity(ApiUser? user) {
    if (user != null && user.universityName.isNotEmpty) {
      if (user.isCustomUniversity) {
        _selectedUniversity = DemoUniversityStore.instance.allUniversities
            .firstWhere((u) => u.id == 'uni_other');
        _customUniCtrl.text = user.universityName;
      } else {
        final norm =
            UniversityOption.normalizeUniversityName(user.universityName);
        final match = DemoUniversityStore.instance.allUniversities.firstWhere(
          (u) => u.id == user.universityId || u.normalizedName == norm,
          orElse: () => UniversityOption.create(
            id: user.universityId.isNotEmpty ? user.universityId : 'uni_custom',
            name: user.universityName,
          ),
        );
        _selectedUniversity = match;
      }
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (_selectedUniversity?.id == 'uni_other') {
      ...
      uniToSend = DemoUniversityStore.instance
          .getOrCreateCustomUniversity(_customUniCtrl.text);
    }
